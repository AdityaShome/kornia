...
def _to_uint8(image: Tensor) -> Tensor:
    ...
    if image.dtype==torch.float32:
        ...
        return (image / 256).to(torch.uint8)
...
def load_image(
    ...
    Return:
        ...
        Image tensor with shape :math:`(C,H,W)` where C can be 1, 3, or 4,
        and dtype depends on desired_type and source image format.
    ...
    if desired_type==ImageLoadType.UNCHANGED:
        ...
        if image.shape[0]==1 and image.dtype==torch.uint8:
            ...
            img_uint8 = _uint16_to_uint8(image)
            if img_uint8.shape[0]==1:
                return img_uint8
            elif img_uint8.shape[0]==3:
                return kornia.color.rgb_to_grayscale(img_uint8)
            elif img_uint8.shape[0]==4:
                gray32 = kornia.color.rgb_to_grayscale(kornia.color.rgba_to_rgb(_to_float32(img_uint8)))
                return _to_uint8(gray32)
        elif image.dtype==torch.float32:
            img_uint8 = _float32_to_uint8(image)
            if img_uint8.shape[0]==1:
                return img_uint8
            elif img_uint8.shape[0]==3:
                return kornia.color.rgb_to_grayscale(img_uint8)
        ...
        if image.shape[0]==3 and image.dtype==torch.uint8:
            ...
            if img_uint8.shape[0]==3:
                ...
                return kornia.color.grayscale_to_rgb(img_uint8)
        elif image.dtype==torch.float32:
            img_uint8 = _float32_to_uint8(image)
            if img_uint8.shape[0]==3:
                return img_uint8
        ...
        if image.shape[0]==3 and image.dtype==torch.uint8:
            ...
            img_uint8 = _uint16_to_uint8(image)
            if img_uint8.shape[0]==3:
                rgba32 = kornia.color.rgb_to_rgba(_to_float32(img_uint8), 0.0)
                return _to_uint8(rgba32)
            elif img_uint8.shape[0]==4:
                return img_uint8
        elif image.dtype==torch.float32:
        ...
        if image.shape[0]==1 and image.dtype==torch.uint8:
            ...
            img_float32 = _uint16_to_float32(image)
            if img_float32.shape[0]==1:
                return img_float32
            elif img_float32.shape[0]==3:
                return kornia.color.rgb_to_grayscale(img_float32)
            elif img_float32.shape[0]==4:
                return kornia.color.rgb_to_grayscale(kornia.color.rgba_to_rgb(img_float32))
        elif image.dtype==torch.float32:
            if image.shape[0]==1:
                return image
                ...
                return kornia.color.rgb_to_grayscale(kornia.color.rgba_to_rgb(image))
        ...
        if image.shape[0]==3 and image.dtype==torch.uint8:
            ...
            img_float32 = _uint16_to_float32(image)
            if img_float32.shape[0]==3:
                return img_float32
            elif img_float32.shape[0]==1:
                return kornia.color.grayscale_to_rgb(img_float32)
        elif image.dtype==torch.float32:
            if image.shape[0]==3:
                return image
